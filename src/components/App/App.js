...
function App() {
  ...
  function handleFilterAllClick(value) {
    setMainCheckbox(!mainCheckbox);
    const changeIsChecked = checkboxes.map((elem) => ({ ...elem, isChecked: value }));
    setCheckboxes(changeIsChecked);
  }

  function handleFilterCheckboxClick(item) {
    if (!item.isChecked) {
      setMainCheckbox(false);
    }
    const changeIsChecked = checkboxes.map((elem) => elem.id === +item.id ? {
      ...elem,
    ...
    <div className="page">
      <main className="content" >
        <TicketList
          tickets={tickets}
          count={count}
          onFilterAllClick={handleFilterAllClick}
          sortType={sortType}
